...
#define SOLE_VERSION                                                           \
  "1.0.1" /* (2017/05/16): Improve UUID4 and base62 performance; fix warnings  \
#define SOLE_VERSION "1.0.0" // (2016/02/03): Initial semver adherence; Switch \
to header-only; Remove warnings */
...
// Generators
uuid uuid0();  // UUID v0, pro: unique; cons: MAC revealed, pid revealed,
               // predictable.
...
#define $windows $yes
    ...
    defined(__MINT__) || defined(__bsdi__)
#include <ifaddrs.h>
#include <net/if_dl.h>
...
static inline std::string put_time(const std::tm *tmb, const char *fmt) {
  std::string s(128, '\0');
  while (!strftime(&s[0], s.size(), fmt, tmb)) s.resize(s.size() + 128);
  return s;
}
}  // namespace std
#endif
...
inline std::string printftime(uint64_t timestamp_secs = 0,
                              const std::string &locale = std::string()) {
  std::string timef;
  try {
    // Taken from parameter
    // std::string locale; // = "es-ES", "Chinese_China.936", "en_US.UTF8",
    // etc...
    ...
    $msvc(localtime_s(&tm, &t);) $melse(localtime_r(&t, &tm);)
        ...
        std::stringstream ss;
    $melse(std::locale lc(locale.c_str()); ss.imbue(lc);) ss
        << std::put_time(&tm, "\"%c\"");
...
inline std::string uuid::pretty() const {
  ...
  uint64_t timestamp =
      ((b & 0x0FFF) << 48) | ((b >> 16) << 32) | a;  // in 100ns units

  ss << "version=" << (version) << ',';

  if (version == 1)
    timestamp =
        timestamp - 0x01b21dd213814000ULL;  // decrement Gregorian calendar

  ss << std::hex << std::nouppercase << std::setfill('0');
  version <= 1 && ss << "timestamp=" << printftime(timestamp / 10000000) << ',';
  version <= 1 && ss << "mac=" << std::setw(4) << (c & 0xFFFF) << std::setw(8)
                     << d << ',';
  version == 4 && ss << "randbits=" << std::setw(8)
                     << (ab & 0xFFFFFFFFFFFF0FFFULL) << std::setw(8)
  ...
  uint64_t tv_nsec;
         ...
         inline int gettimeofday(struct timeval *tv, struct timezone *tz) {
           ...
           if (NULL != tv) {
             ...
             // intervals since Jan 1, 1601 in a structure. Copy the high bits
             // to the 64 bit tmpres, shift it left by 32 then or in the low 32
             // bits.
             ...
             // The Unix epoch starts on Jan 1 1970.  Need to subtract the
             // difference in seconds from Jan 1 1601.
           ...
           if (NULL != tz) {
             ...
             $msvc(_get_timezone(&timezoneSecs); _get_daylight(&daylight);)
                 $melse(timezoneSecs = _timezone; daylight = _daylight;)
...
inline bool get_any_mac(std::vector<unsigned char> &_node) {
  $windows({
    ...
    if (!gotten) return $no("cannot get network adapter list") false;

    if (!found) return $no("no Ethernet adapter found") false;
    ...
    return true;
      ...
      $bsd({
        ...
        for (struct ifaddrs *ifap = ifaphead; ifap; ifap = ifap->ifa_next) {
          if (ifap->ifa_addr && ifap->ifa_addr->sa_family == AF_LINK) {
            struct sockaddr_dl *sdl =
                reinterpret_cast<struct sockaddr_dl *>(ifap->ifa_addr);
        ...
        if (!foundAdapter)
          return $no(
              "cannot determine MAC address (no suitable network adapter "
              "found)") false;
        return true;
      })
          ...
          $osx({
            ...
            return true;
              ...
              $linux({
                ...
                struct sockaddr *sa =
                    reinterpret_cast<struct sockaddr *>(&ifr.ifr_addr);
                ...
                return true;
                  ...
                  $unix({
                    ...
                    struct sockaddr_in *pAddr =
                        reinterpret_cast<struct sockaddr_in *>(&ar.arp_pa);
                    pAddr->sin_family = AF_INET;
                    std::memcpy(&pAddr->sin_addr, *pHost->h_addr_list,
...
inline uint64_t get_any_mac48() {
  ...
  if (get_any_mac(node)) {
    ...
    node.resize(6);
    for (unsigned i = 0; i < 6; ++i) ss << std::setw(2) << int(node[i]);
    uint64_t t;
    if (ss >> t) return t;
  }
  return 0;
...
void tests(const FN &fn) {
  ...
  for (unsigned i = 0; i < numtests; ++i) {
    ...
    if (now != then) {
      ...
      std::cout << '\r' << i << " uuids generated, no collision (" << pct6digs
                << "%)" << std::flush;
    }
    sole::uuid my_uuid = fn();
    assert(all.find(my_uuid) == all.end() &&
           "error: UUIDs just collided! is std::random_device a real random "
           "generator?");
    all.insert(my_uuid);
  }
}
