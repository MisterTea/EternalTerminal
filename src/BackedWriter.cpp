...
void BackedWriter::backupBuffer ( const void* buf, size_t count ) {
  if ( immediateBackup.size ( ) > 0 && immediateBackup.back ( ).length ( ) + count < BUFFER_CHUNK_SIZE ) {
...
BackedWriterWriteState BackedWriter::write ( const void* buf, size_t count ) {
  ...
  while ( true ) {
    ...
    ssize_t result = socketHandler->write ( socketFd, ( ( char* ) &s[ 0 ] ) + bytesWritten, count - bytesWritten );
    if ( result >= 0 ) {
      bytesWritten += result;
...
std::string BackedWriter::recover ( int64_t lastValidSequenceNumber ) {
  ...
  for ( auto it = immediateBackup.rbegin ( ); it != immediateBackup.rend ( ); ++it ) {
    if ( bytesSeen + ( int64_t ) it->length ( ) < bytesToRecover ) {
      ...
      VLOG ( 1 ) << "Reached end: " << bytesSeen << " " << it->length ( ) << " " << bytesToRecover << " "
                 << bytesToWrite;
      ...
      if ( int64_t ( s.length ( ) ) != bytesToRecover ) {
        LOG ( FATAL ) << "Error, did not recover the correct number of bytes: " << bytesToRecover << " "
                      << s.length ( );
      }
      return s;
    }
