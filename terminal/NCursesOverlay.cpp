...
namespace et {
  ...
  NCursesOverlay::~NCursesOverlay() {
    for (const auto& it : windows) {
      if (it.use_count() != 1) {
        LOG(FATAL) << "Tried to delete the overlay with a dangling window reference";
      }
    }
    windows.clear();
    curs_set(1);
  ...
  shared_ptr<NCursesWindow> NCursesOverlay::createWindow(const TerminalInfo& info, bool showBorder) {
