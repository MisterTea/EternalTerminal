...
TEST_CASE("Connection recover can read the peer catchup first",
  ...
  // The peer follows the server's order: it sends its catchup, then reads.
  ...
  std::thread remote([&]() {
    ...
    // A connection that reads first sends nothing until our catchup arrives.
  ...
  REQUIRE(conn.recoverPublic(reconnect[0], /*readPeerCatchupFirst=*/true));
  remote.join();
