// Drives the real etctl binary against a real bash wired behind a
// ControlConsole
// + ControlListener via pipes.  bash stands in for the remote shell, so this
// verifies the full interactive data plane end-to-end: run() clean output +
// exit codes, writeln, read, and expect, all against a program that actually
...
TEST_CASE("EtctlRunAgainstRealShell", "[EtctlRun]") {
  ...
  {
    // Multi-line command: the body runs as one unit (eval of a here-doc), so a
    // var set on the first line is still visible on the last (no early line
    // submission).
  ...
  }
  {
    ...
    // The body is here-doc data fed to eval, so the parse error surfaces at
    // eval runtime; the end marker still prints and run returns a real,
    // non-timeout code.  The old brace-group wrap let a body parse error
  ...
  }
  {
    ...
    RunResult r =
        runEtctl("expect " + name + " MARKER_zzz --timeout 10 --from-start");
