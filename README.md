...
symbolic link, and its absolute path may contain only ASCII letters, digits,
`/`, `.`, `_`, and `-`; OpenSSH does not quote this path when constructing its
implicit ProxyJump command. Pass `--ssh-config none` or the equivalent
`--no-ssh-config` to disable both user and system SSH configuration entirely.
